...
def check_hash(hash_string=None,john=False,Hashcat_mode=False,extended=False):
    ...
    hashes_list = []
    hashType = ""
    identified_modes = hashID.identifyHash(hash_string)
    for mode in identified_modes:
        if not mode.extended or extended:
            count += 1
            hashType += u"{0}".format(mode.name)
            if Hashcat_mode and mode.hashcat is not None:
                hashType += " [Hashcat Mode: {0}]".format(mode.hashcat)
            if john and mode.john is not None:
                hashType += " [JtR Format: {0}]".format(mode.john)
        hashes_list.append(hashType)
        hashType = ""
    if count == 0:
        hashes_list.append(u"Unknown hash")
    hashes_list = [h for h in hashes_list if h]
    return hashes_list
